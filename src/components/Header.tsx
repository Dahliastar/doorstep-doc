      ...
      <div className="container mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          ...
          {!loading && (user ? <div className="flex items-center gap-4">
                ...
                <Button variant="ghost" size="sm" onClick={() => navigate('/profile')} className="text-primary hover:bg-primary/10">
                  <User className="h-4 w-4 mr-2" />
                  Profile
                </Button>
                <Button variant="ghost" size="sm" onClick={() => navigate('/dashboard')} className="text-primary hover:bg-primary/10">
                  <BarChart3 className="h-4 w-4 mr-2" />
                  Dashboard
                </Button>
                <Button variant="outline" size="sm" onClick={signOut} className="text-primary border-primary hover:bg-primary hover:text-white">
                  <LogOut className="h-4 w-4 mr-2" />
                  Sign Out
                </Button>
              </div> : <>
                <Button variant="outline" size="sm" onClick={() => navigate('/auth')} className="text-primary border-primary hover:bg-primary hover:text-white">
                  Sign In
                </Button>
